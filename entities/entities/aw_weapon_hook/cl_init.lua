...
local state_models = {}
state_models[0] = "models/aw_hook/aw_hook.mdl"
state_models[1] = "models/aw_hook/aw_hook_s.mdl"
...
function ENT:CheckState()
	if self.CurrentState == self:GetHookState() then return end
	local part_id = self:GetPartID()
	if world_ships[self:GetAWTeam()] == nil then return end
	local part = world_ships[self:GetAWTeam()].parts[part_id]
	if part == nil then return end
	self.CurrentState = self:GetHookState()
	part.model = state_models[self.CurrentState] or "models/aw_hook/aw_hook.mdl"
end
